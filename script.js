...
class LanguagePreference {
    ...
    init() {
        ...
        if (!storedPreference) {
            const targetPage = this.getTargetPage(normalizedKey, this.defaultLanguage);
            if (targetPage && targetPage !== currentPath) {
                this.storePreference(this.defaultLanguage);
                window.location.href = this.buildTargetUrl(targetPage);
                return;
            }

            this.storePreference(currentLanguage || this.defaultLanguage);
